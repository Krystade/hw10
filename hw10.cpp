...
vector::vector(vector&& source) : vsize{source.vsize}, elem{source.elem}// move constructor
// copy source elem and vsize only, no need for copy operation
{
	source.elem = NULL;		// now that source vector contents have been moved, empty the vector
	source.vsize = 0;
}
    ...
    // make this vector a copy of the rhs (i.e. source)
{
    ...
    delete [] elem;
    delete [] pD;                    // deallocate old space
	...
	// move rhs (i.e. source) to this vector
{
	...
	rhs.elem = NULL;	// empty the rhs vector
	rhs.vsize = 0;
	return *this;		// return a self-reference
}
...
int main()
{
	...
	cout << "*      Programmed by Jack Schmid     * " << endl;
	cout << "*      CS1C 3/23/2020 4:00 Pm        * " << endl;
	cout << "************************************** " << endl;
	cout << endl;
    ...
    vector v2 = v;      // [1.1] copy v to v2: what happens here?
                        // What happens here is that it copies v to v2. The copy operation is called to initialize v2 with the values of v
    ...
    v3 = v4;            // [2.1] assign v4 to v3: what happens here?
    					// v4 is copied to v3. The values of v4 replace the values of v3 and v3's old elements are deallocated.
    ...
    //v5 = v5;            // [2.2] self assignment: what happens here, any problems?
    					// The vector of size 10 v5 is assigned to itself with copying. pointer is reset and old elements are deallocated. Thje program runs fine
...
A copy operation is used when an instance of one object needs to be assigned to another.
A move operation is more efficient than the copy operation. Have to do with lvalues and rvalues. rvalues have mutable references bound to them, the temporary values.
...
What happens when the temp vector is returned from the fill_doubles function and assigned to v6 in main?
The move constructor is invoked.

How does this result in improved performance?
By using the move constructor we can just reassign the addresses to v6.
*/
